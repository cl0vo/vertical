...
def extract_wav(ffmpeg: str, source: Path, target: Path) -> None:
    cmd = [ffmpeg, '-y', '-i', str(source), '-vn', '-ac', '1', '-ar', '16000', '-c:a', 'pcm_s16le', str(target)]
...
def detect_pulses(wav_path: Path, min_silence: float = 0.10) -> list[Pulse]:
    ...
    rms = np.array([np.sqrt(np.mean(data[i:i+frame] ** 2) + 1e-8) for i in range(0, max(1, len(data)-frame), hop)])
    if rms.size == 0:
        return []
    ...
    for i in range(1, len(active) - 1):
        if not active[i] and active[max(0, i-min_gap):i].any() and active[i+1:min(len(active), i+min_gap+1)].any():
            active[i] = True
    pulses: list[Pulse] = []
    start = None
