...
# Canonical PNG measured from Vertical overlay Arara(1).png.
REFERENCE_WIDTH = 941
...
# Exact transparent holes in the canonical PNG.
MAIN_CONTENT = Rect(13, 592, 913, 553)
BRAINROT = Rect(13, 1256, 501, 402)


def scale_rect(rect: Rect, target_width: int, target_height: int) -> Rect:
    """Scale canonical pixel coordinates to a rendered Reel without drift."""
...
def canonical_layout(target_width: int, target_height: int) -> tuple[Rect, Rect]:
    """Return main-content and lower brainrot rectangles for a Reel size."""
    return (
        scale_rect(MAIN_CONTENT, target_width, target_height),
        scale_rect(BRAINROT, target_width, target_height),
    )
